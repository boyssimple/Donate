...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(tableView == _provinceTableView) {
       ...
       name = [[_provinceArray objectAtIndex:row] objectForKey:@"name"];
        //城市数组
        [self getData:_provinceArray[_provinceLine][@"region_id"] withType:1];
        ...
        //区的
        [self getData:_cityArray[_cityLine][@"region_id"] withType:2];
...
- (void)completionAddress {
    ...
    NSString * provinceCode = _provinceArray[_provinceLine][@"region_id"];
    NSString * cityCode = _cityArray[_cityLine][@"region_id"];
    NSString * areaCode = _areaArray[_areaLine][@"region_id"];
...
//    self.completion(_addressString, _addressCode);
    self.completion(provinceName,cityName,areaName,provinceCode,cityCode,areaCode);
    [self hiddenAddress];
}
