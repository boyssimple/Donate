...
- (void)setSelected:(BOOL)selected animated:(BOOL)animated
{
    if (self.canShowCallout == NO )return;
        if (selected)
        {
            if (_CalloutView == nil)
            {
...
//     CGFloat Width = [titleText sizeWithFont:[UIFont systemFontOfSize: 13] constrainedToSize:CGSizeMake(CGFLOAT_MAX, 21) lineBreakMode:NSLineBreakByWordWrapping].width;
    ...
    _titleLable.numberOfLines = 2;//根据最大行数需求来设置
    _titleLable.lineBreakMode = NSLineBreakByTruncatingTail;
    CGSize maximumLabelSize = CGSizeMake(100, 9999);//labelsize的最大值
    //关键语句
    CGSize expectSize = [_titleLable sizeThatFits:maximumLabelSize];
    //别忘了把frame给回label，如果用xib加了约束的话可以只改一个约束的值
    _titleLable.frame = CGRectMake(0, 0, expectSize.width, expectSize.height);

    [_contentView setFrame:CGRectMake(0, 0, expectSize.width, expectSize.height)];
    [_titleLable setFrame:CGRectMake(0,0, expectSize.width, expectSize.height)];
