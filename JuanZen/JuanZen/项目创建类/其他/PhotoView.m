...
@interface PhotoView ()<UIScrollViewDelegate,UIAlertViewDelegate>{
    MBProgressHUD *HUD;
}
...
-(id)initWithFrame:(CGRect)frame withPhotoUrl:(NSString *)photoUrl{
    ...
    if (self) {
        ...
        if (!isCached) {//没有缓存
            ...
            [self.imageView sd_setImageWithURL:[NSURL URLWithString:photoUrl] placeholderImage:DEFAULTIMAGE options:0 progress:^(NSInteger receivedSize, NSInteger expectedSize){
...
// 长按
- (void)longGestureTap:(UILongPressGestureRecognizer *)longGestureRecongnizer {
    if ([longGestureRecongnizer state] == UIGestureRecognizerStateBegan) {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil message:@"保存图片?" delegate:self cancelButtonTitle:@"否" otherButtonTitles:@"是", nil];
        [alertView show];
    }
}
...
- (UIImageView *)imageView {
    ...
    if (_imageView==nil) {
        ...
        //添加手势
        UILongPressGestureRecognizer *longTap = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(longGestureTap:)];
        UITapGestureRecognizer *singleTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(handleSingleTap:)];
        UITapGestureRecognizer *doubleTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(handleDoubleTap:)];
        UITapGestureRecognizer *twoFingerTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(handleTwoFingerTap:)];
        
        longTap.minimumPressDuration = 0.3; //定义按的时间
